...
object DisciplinaryProblem1 extends Problem[Double] {
   ...
   *
   * @param  x array of an individual
   ...
   * @return
   */
  override def getProblemName: String = "test-optimization-disciplinary-1"
  ...
  override def checkConstraint(x: Array[Double]): Boolean = {
    ...
    val constraints1 = x(1) - 100 * Math.sin(0.1 * x(0) - 0.5)
    constraints1 <= convergenceConditionThreshold
  }
}
